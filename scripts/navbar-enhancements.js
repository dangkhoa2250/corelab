...
(() => {
  ...
  const normalizeSitePath = (path) => {
    let nextPath = path.replace(/^https?:\/\/[^/]+/, '');
    // Strip site subpath prefix (e.g. /corelab/) using quarto:offset meta tag
    const offset = document.querySelector('meta[name="quarto:offset"]')?.getAttribute('content') || '';
    if (offset) {
      const upCount = (offset.match(/\.\.\//g) || []).length;
      if (upCount > 0) {
        const parts = nextPath.replace(/^\//, '').split('/');
        const siteRootParts = parts.slice(0, parts.length - upCount - 1);
        if (siteRootParts.length > 0) {
          const siteRoot = '/' + siteRootParts.join('/') + '/';
          nextPath = nextPath.replace(siteRoot, '/');
        }
      }
    }
    nextPath = nextPath.replace(/^\//, '');
    if (nextPath.endsWith('/')) nextPath += 'index.html';
    nextPath = nextPath.replace(/\.qmd$/, '.html');
  ...
  const setupCourseChapterPager = () => {
    ...
    const lang = getCoursePageLang();
    const labels = {
      en: { previous: 'Previous chapter', next: 'Next chapter', start: 'Start course', overview: 'About this course' },
      vi: { previous: 'Bài trước', next: 'Bài tiếp theo', start: 'Bắt đầu khóa học', overview: 'Giới thiệu' },
      ja: { previous: '前のチャプター', next: '次のチャプター', start: 'コースを開始', overview: 'このコースについて' }
    }[lang] || { previous: 'Previous chapter', next: 'Next chapter', start: 'Start course', overview: 'About this course' };

    // Prepend the Overview page to chapters list so Chapter 1 can navigate back to it
    const offset = document.querySelector('meta[name="quarto:offset"]')?.getAttribute('content') || '';
    chapters.unshift({
      text: labels.overview,
      href: offset + coursePaths[lang],
      path: coursePaths[lang]
    });
    ...
    if (isOverviewPage) {
      pager.innerHTML = [
        ...
        renderItem(chapters[1], 'next', labels.start)
      ].join('');
    } else {
